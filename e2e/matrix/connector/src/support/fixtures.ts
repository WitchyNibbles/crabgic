 ...
 * either a well-known sentinel shape (matching `@eo/renderer`'s
 ...
 * `@eo/renderer/src/unicode-defense.ts`'s own documented convention — "never
 * a literal invisible glyph in source").
 */

/**
 * AWS-style access key sentinel — `AKIA` + exactly 16 uppercase
 * alphanumerics, matching `@eo/renderer`'s `AKIA[0-9A-Z]{16}` pattern
 ...
 * hyphens, matching `@eo/renderer`'s `sk-ant-[A-Za-z0-9-]{10,}` pattern.
 ...
 * `@eo/renderer/src/unicode-defense.ts`'s own convention for its curated
 ...
 * (`@eo/renderer/src/unicode-defense.ts`'s own CONFUSABLE_ENTRIES table
