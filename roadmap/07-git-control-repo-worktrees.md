...
- **Control clone:** `git clone --no-local` per project into `$XDG_CACHE_HOME/crabgic/<project-hash>/git-control/` (cache-root convention pinned in 04); never shared object alternates; fetch strategy for target-ref updates.
...
- **Git identity:** per-worktree `user.name "Crabgic"` + configured service email, set at worktree-creation time — every commit a worker makes already carries it before 08 ever inspects the tree.
...
- **Control clone** — on-disk control repository at `$XDG_CACHE_HOME/crabgic/<project-hash>/git-control/` (Gap 14). Consumed by 08 (`git fetch <control-repo> <ref>:refs/heads/<branch>`, run in the user's repo, at publication time) and by 23 (Git-invariance matrix harness, reused directly at this exact path).
...
- **Git identity guarantee** — every worktree this phase creates already carries `user.name "Crabgic"` + the configured service email before any commit lands in it. 08's commit rendering and its belt-and-suspenders attribution assertion depend on this already being true; neither re-configures identity.
...
- [ ] Control clone resolves at `$XDG_CACHE_HOME/crabgic/<project-hash>/git-control/` — a path-convention test, not just documentation.
