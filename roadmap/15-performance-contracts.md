...
- **Release re-run record** — the A/B runner's output when 23 re-runs it on a quiet host for the release candidate's real verdicts (23's own exit criterion) is archived as a phase-23 evidence record at `docs/evidence/phase-23/perf-contract-rerun.json`, or handed to the consuming check out-of-checkout through `$EO_PERF_CONTRACT_RERUN_RECORD`, an absolute path to that file; the path convention, the `EO_<SUBJECT>` override naming, and the requirement that a missing or malformed record FAIL that one release-gate item rather than throw are `docs/interface-ledger.md`'s Gap 16, not this phase's invention.
