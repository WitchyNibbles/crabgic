...
- [x] `wikiMarkupRenderProfile` output passes 17's blocking-artifact-lint corpus — golden-file diff test, zero exceptions. — **Evidence (2026-08-06), closed by remediation:** both substantive conjuncts had no bearer — all three `lint()` calls in this phase's serializer suite lint the INPUT candidate and then convert, and the "golden corpus" that file names is a 7-item hand-written array declared inside it. `packages/connectors-jira/src/resource-client/datacenter/wiki-markup-corpus.test.ts:97-98` runs every `expect: "ok"` fixture of `packages/renderer/fixtures/corpus/` through `lint(adfDocumentToWikiMarkup(toADF(candidate)), …)`, and `:143-145` diffs the converted output byte-for-byte against a committed golden per fixture under `packages/connectors-jira/fixtures/wiki-golden/`, with `:129-133` pinning the golden set equal to the corpus set. CI run [31082480661](https://github.com/WitchyNibbles/crabgic/actions/runs/31082480661) job 92554240575 line 390 shows the new suite executing at 69 tests. The assertion found a real defect: `toADF` never emitted `hardBreak`, so every templated multi-line artifact collapsed to one line and `valid-pr-body`, `valid-jira-milestone-comment` and `valid-review-comment` each linted BLOCK(schema-validation) after the round trip — three artifacts that pass the lint as authored would have gone on the DC wire malformed. Reverting the fix leaves 28 failed / 765 passed; rendering `hardBreak` as a space leaves 27 failed / 766 passed. `attack-remote-image` is a pinned residual asserted at `:122-123`: the ADF safe subset has no image node, so the construct is destroyed rather than laundered. This closes criterion 4 only — criteria 1-3's cassette conjuncts remain open and need a licensed Data Center instance.
