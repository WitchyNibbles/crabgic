...
- **`EnvelopePolicy` (added 2026-07-28 — ledger Gap 18):** the standing approval every dispatch is checked
  ...
  the implementation is 03's, as the security keystone. Since 2026-08-06 the envelope also carries the derived `provisionalBudgetHash`, and `PerformanceContract`'s enforced variant carries `approvedEnvelopeHash` (ledger Gap 22) — a binding REFERENCE, deliberately not an authority dimension, so it is invisible to the containment predicate above by design.
