...
- [ ] `GATEWAY_MCP_SERVER_NAME` is the sole definition site of the literal `"crabgic_gateway"` — a repo-wide grep/golden-value CI check fails if the literal appears a second time under `packages/*`. — **Left unticked 2026-08-01, defect filed:** `docs/evidence/criteria-closeout/defects/02-gateway-literal-scan-scope.md`. The golden-value half is met (`packages/contracts/src/gateway/server-name.test.ts:76`), and the scan works within its scope — a negative control appending the literal to `packages/journal/src/index.ts` makes `:95 expect(violations).toEqual([])` fail. But the scan walks only `packages/*/src/**/*.ts`, and the criterion says `packages/*`: `packages/plugin/.mcp.json` hand-types `"crabgic_gateway"` as the shipped plugin manifest's server key, bound to the constant by no offline test (only the `@live` `plugin-load.live.test.ts:51`), and `packages/engine-claude/src/live/fixtures/stub-mcp-server.mjs` hand-types it once more. So the literal *does* appear a second time under `packages/*` while the check stays green. The remaining occurrences are derived, verified during this pass: the `engine-core` goldens regenerate through `write-goldens.ts` for an empty diff, the `engine-claude` goldens are byte-asserted against freshly generated output, and the *installer's* `.mcp.json` entry is correctly built from the imported constant (`packages/cli/src/installer/mcp-entry.golden.test.ts:24-29`). Not reworded to the scan's real scope, because that would retire a guarantee §In scope states in its own words ("no phase hand-types the literal a second time") and that a shipped artifact is currently violating. Remedy is S-sized and needs neither CI nor the live engine. Transcript: `docs/evidence/phase-02/closeout-c8-gateway-literal-scan.txt`.
