 ...
 * the real, pinned Claude Code engine, plus roadmap/10's
 * `packages/plugin/src/live/*.live.test.ts` plugin-load smoke test (which
 * reuses this same `engine-live` job per 10 §Interfaces consumed). Invoked by
 * `npm run test:live` and the `engine-live` CI job — never by the default gate
