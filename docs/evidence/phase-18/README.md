...
> _Amended 2026-08-05 (PR #95):_ this guard's secret scan originally saw only `extractPlainText`'s output (`node.text`). A secret in an `https:` link mark's `attrs.href` query string, or in an unknown extra member that `validateAdfSafeSubset`'s `type`/`marks`/`content` walk never visits, passed it and reached the outbound body on **both** providers. The scan now additionally runs over the document's whole JSON serialization — which on Cloud _is_ the outbound body — while **keeping** the extracted-text scan, since JSON escaping defeats the `\s`-bearing patterns. Evidence: `docs/evidence/phase-18/adf-serialization-secret-scan.txt`.
