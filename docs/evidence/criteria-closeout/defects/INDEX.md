...
- **`open` means "this record does not evidence a remedy" — not "no remedy exists."** The `24-`
  row is exactly that case: the fix shipped and is annotated in the production source, but the
  record was never given an addendum, so by this table's own vocabulary it is not `fixed`.
...
| `24-daemon-requirements-registry-unwired.md`              | 24    | 5, 6                  | The shipped daemon composed no requirements registry, so seal verification resolved an empty set for every work unit. PR #85 wired it and annotated the fix in the production source; **this record has no addendum**. | open        |

## Counts

29 records: **1 fixed**, **7 owner-gated**, **21 open**.
