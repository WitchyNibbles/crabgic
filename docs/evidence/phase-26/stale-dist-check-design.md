...
the cache-PRESENT case alone and §5's `meta-checks` bullet asserts four `not-run` entries
...
preceded by the PRODUCER of each artifact the `not-run` reasons name, which is
`npm run build` for EVERY absence spelling §3.2, §3.3, §3.4 and §3.5 mandate. ⚠️ **That is
...
this rule governs are bounded rather than omitted: `packages/cli unbuilt` names no artifact
...
- **Rows 16-18, 26-36, 38 and 39 assert `formatFindings`' RENDERED text, never the return value.** Rows **38** and **39** are round 14's, both in row 34's MUTANT-FORMATTER shape; row **37** is deliberately NOT in this group, its subject being a `unitState` verdict, for which the return value is the only surface (the reason rows 12 and 23 are out too). Round 15's four are all out for one of those two reasons: row **40** is a DIRECT-CALL row in rows 12 and 23's shape, and rows **41**, **42** and **43** are verdict rows in row 37's shape. Round 16's two are out for the same two reasons: row **44** is a verdict row in row 37's shape — its subject is `checkUnitFreshness`'s and `checkOrphans`' return over one fixture — and row **45** is a DIRECT-CALL row in rows 40 and 41's shape, its mutation column calling `checkBundleFreshness` directly. Round 17's three are out for the same two reasons: row **46** is a DIRECT-CALL row in rows 12, 23 and 40's shape, its mutation column calling `checkBundleFreshness` and `checkPluginAssets` directly because §3.2's and §3.4's not-run causes are reached by one fixture; and rows **47** and **48** are verdict rows in rows 37's and 44's shape — their subject is `checkUnitFreshness`'s return over one fixture and the input SET feeding it, not the reporter, the rendering of the grouped `stale-unit` line being row 28's subject and already pinned by equality (`OP15-1`). Row **2**, whose verdict round 16 tightened from `×N` to an equality on `Unit.dir` (`CF16-3`), was never in this group and does not join it: the RENDERING of the grouped `stale-unit` line is row 28's subject and is already pinned by equality (`OP15-1`).
  ...
  remedy plan's first line — row 39 as printed by a run holding ZERO findings (round 14) — and row 38 additionally on the ABSENCE of any `check-stale-dist: ⚠️ coverage: ` line, its fixture being the one in this group whose four coverage entries are all `full` (round 16, `OP16-4`).** For rows 16, 17 and 26 the
...
deliberate exception, its whole subject being one absent member. ⚠️ **Row 45 CONFORMS to
