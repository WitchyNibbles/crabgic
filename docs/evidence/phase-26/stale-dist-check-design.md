...
| `scripts/check-stale-dist.test.mjs` | ~680         | colocated suite (§6) — **45** battery rows, the false-negative battery, the mutant-proved wiring assertions and the live smoke test                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
`inlinedUnitsFromMetafile` occurs **three** times in this design and none of them inside §6.
...
- **Row numbers are cited from OUTSIDE this section, so nothing here is renumbered
  ...
  **43** BELOW A BLANK LINE — so not one of the four was on the page until round 16
  (`CF16-1`/`OP16-1`) deleted it — and round 16 appended **44** and **45**; nothing
...
LAST** — rows 3, 5, 7, 8, 10, 11, 16, 17, 19, 20, 21, 22, 24, 25, 26, 32, 33, 34, 36, 37, 38, 39, 40, 41, 42, 45 and 29's two cli fixtures (its `bundle-provenance-missing` and `stale-shipped-declarations` states): a chunk, the three entry
...
- **`C16-2`, medium — §3.2's central rule was discharged by nothing.** "The inlined set is
  ...
  2026-08-21 `inlinedUnitsFromMetafile` occurs **three** times in this design with none
  inside §6. A `checkBundleFreshness` whose inlined set is the transitive closure of
...
- **`CF16-3`, medium — requirement 6's SOLE non-vacuity element was satisfied at N=1.** §6
  ...
  `stale-unit ×1`, and `×N` accepts it. Nothing else discriminates — re-derived 2026-08-21,
  `extends` occurs **zero** times anywhere in §6, row 1 is src-side, row 3's trigger is
