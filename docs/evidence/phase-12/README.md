...
| `capability.audit`/`capability.approve` resolve over the shared `eo_gateway` registry against a stub MCP client; `capability.approve` rejects a call lacking a pre-minted `trust approve` token | `src/mcp/tool-definitions.test.ts` (both tools visible over real stdio `tools/list` via 09's own `createToolRegistry`/`startGatewayMcpServer`, duplicate-registration rejection); `src/mcp/capability-audit-handler.test.ts`; `src/mcp/capability-approve-handler.test.ts` (genuine mint→verify→approve flow; fails closed for: no token, wrong digest, replayed/consumed token, wrong subject-kind token — the "model-self-approval" seeded threat) — **Correction (2026-08-06), the row above left verbatim.** `src/mcp/tool-definitions.test.ts` no longer exists: it was relocated to `packages/cli/src/gateway-mcp/detect-tool-definitions.test.ts` in `5c21a0f` and then DELETED in `c39292c`, together with the hand-rolled stdio server it drove. Its `tools/list` assertion was re-established on 2026-08-06 by `packages/cli/src/gateway-mcp/capability-tools-over-stdio.test.ts`, which resolves both tools over the REAL production registry (`buildRealGatewayToolRegistry`) and a REAL MCP SDK server (`connectGatewayMcpStdio` over `PassThrough` streams) — strictly stronger than the "stub MCP client" the criterion's wording asks for. Read the registry name `eo_gateway` above as `crabgic_gateway`, per this file's own 2026-08-02 rename annotation. |
...
register these two handlers against it. **Discharged 2026-08-06.** 09's hand-rolled stdio server was retired in `c39292c` rather than extended, and the MCP SDK server that replaced it does dispatch `tools/call`. `packages/cli/src/gateway-mcp/capability-tools-over-stdio.test.ts:214-221` drives a real `tools/call` of `capability.approve` through it and asserts the refusal, with an unknown-tool control at `packages/cli/src/gateway-mcp/capability-tools-over-stdio.test.ts:240` so the refusal cannot pass for the wrong reason. So "there is no real dispatch path from an actual MCP tool CALL to these handlers today" is no longer true. **One precision the new suite states about itself:** the refusal it observes comes from the DIGEST guard, not the token check — mutating only the token seam leaves it green, and only removing BOTH guards reddens it (measured: 1 failed / 18 passed). The criterion's token clause therefore stays borne by `packages/detect/src/mcp/capability-approve-handler.test.ts:93`.
