...
## Full live suite, end to end (2026-07-28, final)

`CRABGIC_LIVE=1 vitest --config vitest.live.config.ts` — the whole `@live` suite in one
run, against real bounded workers on `claude` **2.1.220**:
...
Zero failures. This is the first time the suite has been green end to end on this branch,
and it took **four probe fixes** to get there — every one of which was a test asserting
something the engine does not produce, and every one only reachable by spawning workers:

| Probe                              | Was asserting                                                    | Now                                                  |
| ---------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------- |
| `envelope-conformance` path-escape | Turn exhaustion as a **containment** failure                     | 8 turns; exhaustion is explicitly `INCONCLUSIVE`     |
| `crash-recovery` fork              | Turn exhaustion as a **fork** failure (deterministic, not flaky) | 6 turns; same relabelling                            |
| `plugin-load` subagent             | A name its own prompt told the model not to print                | Prompt asks for the name                             |
| `sandbox-containment` ARM 3        | `refused`, against the repo's **own recorded determination**     | Pins the recorded verdict; fails on drift either way |

The last one is the most instructive. It asserted the opposite of what
`sandbox-containment-determination.json` and `engine-baseline.md` §14.2 both record, so it
failed on every run against a fact the project had already written down. A probe that
always fails carries no signal — it teaches a reader to skip the suite, and then a real
failure gets the same shrug.

Phase 06's `path-anchor` determination was also regenerated by this run and re-committed:
**every verdict byte-identical**, only `probedAt` and the ephemeral scratch paths moved.
Under the engine-fact-drift ground rule a probe reproducing its recorded verdicts at a
newer engine is evidence worth keeping, and the timestamp is what carries it.
