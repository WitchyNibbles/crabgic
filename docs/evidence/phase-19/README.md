...
> _Amended 2026-08-05 (PR #95):_ because Data Center's only content guard is this **shared** `assertSafeAdfDocument`, the same one-line widening also closed DC's `[text|href]` wiki-markup path, where a link href lands literally in the outbound string. DC is pinned by its **own** apply-boundary test asserting absence from `request.body`, the typed kind, and `jira-datacenter` attribution — rather than inheriting Cloud's coverage. Evidence: `docs/evidence/phase-18/adf-serialization-secret-scan.txt`.
