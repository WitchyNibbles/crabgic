...
**Re-baseline (2026-07-24, engine 2.1.218):** re-run of `spikes/07-ratelimit.mjs` (itself still zero live API calls, by design — it only scans the other scripts' just-regenerated committed fixtures). Same policy as the original pass: no dedicated live trigger against the owner's subscription. The 2.1.218 re-run of spikes 02–05 incidentally produced **16 `rate_limit_event` messages again**, same count as the original pass, but this time all 16 collapse to a **single distinct `rate_limit_info` payload** (see below) rather than the original pass's 4 distinct payloads — an artifact of this re-run's shorter probe session hitting one steady utilization band, not a schema change. The message envelope and field set are unchanged.
...
**Re-baseline pass (2026-07-24, engine 2.1.218) — distinct `rate_limit_info` payload observed, verbatim (all 16 captured events collapsed to this one value):**

```json
{"status":"allowed_warning","resetsAt":1785243600,"rateLimitType":"seven_day","utilization":0.93,"isUsingOverage":false,"surpassedThreshold":0.75}
```

Same field set as the original pass (`status`, `resetsAt`, `rateLimitType`, `utilization`, `isUsingOverage`, `surpassedThreshold`); `status` and `rateLimitType` values (`allowed_warning`/`seven_day`) are within the same enum observed before. **No schema delta** — this is a narrower sample (one utilization band instead of four), not a changed shape.
...
All paths relative to repo root. **Re-baseline note (2026-07-24):** every fixture listed below (except this section's own list of filenames, which is unchanged) was regenerated at engine 2.1.218 by the phase-23 re-baseline pass; each holds only that FINAL 2.1.218 run per the header's standing rule ("reflect only the FINAL run of each script"). All were re-scanned for `sk-ant-*`/OAuth-token-blob/`$HOME`-path substrings with zero hits (§1 covers the one fixture — `01-auth.verdicts.json` — that required a script fix to stay clean after the handoff-file resolution).
