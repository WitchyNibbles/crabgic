...
describe("renderJiraMilestoneComment", () => {
  it("assembles the Outcome/Evidence/Risk/Next/Ref shape and passes lint", () => {
    ...
    expect(lint(text, "jira_milestone_comment", DEFAULT_COMMUNICATION_POLICY)).toEqual({ ok: true });
  });
...
describe("renderGrafanaAnnotation", () => {
  it("assembles the pipe-delimited shape and passes lint", () => {
    ...
    expect(text).toBe("resolved | api-gateway | rolled back v2.3 | evidence=https://ci.example.com/2");
...
describe("renderPrTitle", () => {
  it("enforces ≤72 chars and 'type(scope): outcome', golden-proven against the commit-subject convention", () => {
    const text = renderPrTitle({ type: "feat", scope: "renderer", outcome: "add blocking lint pipeline" });
  ...
  it("renders without a scope", () => {
    expect(renderPrTitle({ type: "fix", outcome: "correct the parser" })).toBe("fix: correct the parser");
  });
