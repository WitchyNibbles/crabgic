 ...
 * convention is defined and parsed back.
 *
 ...
 * exact primitive — never a parallel HMAC implementation. Importing it from
 * the CLI package closed a dependency cycle (`cli -> learning -> gates ->
 ...
 * so callers keep passing one directly with no adapter.
 */
export interface ApprovalTokenMintSink {
  appendEntry(input: {
    readonly type: Extract<JournalEntryType, "approval_token_mint">;
    readonly payload: { readonly tokenId: string; readonly scope: string };
  }): Promise<unknown>;
}
 ...
 * instance that minted the token, which breaks across a real process
 * boundary (the `run` CLI invocation that mints vs. the separately-spawned
 * `gateway mcp` stdio process that later calls `contract.approve`). Purely
 * additive: no existing behavior of this module changes.
 */
...
export interface ApprovalTokenMinterOptions {
  ...
  readonly journal?: ApprovalTokenMintSink;
}
...
export class ApprovalTokenMinter {
  ...
  readonly #clock: () => number;
  readonly #ttlMs: number;
  readonly #journal: ApprovalTokenMintSink | undefined;
