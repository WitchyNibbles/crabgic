...
while (round <= budget && !closed) {
  ...
  const dispatched = await agent(
    [
      ...
      "",
      "⚠️ EACH VERDICT NEEDS ITS ENVELOPE. `ReviewVerdictSchema` requires six",
      "fields the reviewer does not produce, and a verdict without them is",
      "REFUSED whole — findings and attestations included. Add to each:",
      `  - \`schemaVersion\`: 1`,
      "  - `id`: a fresh uuid, one per lens (never reused across lenses);",
      "  - `createdAt`: an ISO-8601 timestamp of now;",
      `  - \`stage\`: "${stage}"`,
      `  - \`artifactRef\`: "${artifactRef}"`,
      `  - \`round\`: ${String(round)}`,
      "These are the loop's facts, not the reviewer's judgement, so adding them",
      "edits nothing that was reviewed. Every submitter before this instruction",
      "existed rediscovered the refusal by hitting it.",
      "",
