 ...
 * reuses `@crabgic/engine-claude`'s own constants directly rather than
 * duplicating them (unlike `packages/cli`'s pre-existing `doctor/checks/
 * engine-version.ts`, which deliberately does NOT import `@crabgic/engine-claude`
...
/** `@crabgic/contracts` exports the schema but not a standalone named type for this one entry kind (only the discriminated union has a name) — derived locally rather than widening the import surface there. */
export type PluginCapabilityEntry = z.infer<typeof PluginCapabilityEntrySchema>;

export const PLUGIN_CAPABILITY_NAME = "crabgic";
...
/** Builds the digest-pinned `plugin` entry, schema-validated against `@crabgic/contracts`'s `PluginCapabilityEntrySchema`. Starts `pending` — quarantine review (12) resolves it to `approved`/`rejected`; this phase never self-approves. */
 ...
 * `@crabgic/engine-claude`'s `TESTED_ENGINE_VERSION` and the same
