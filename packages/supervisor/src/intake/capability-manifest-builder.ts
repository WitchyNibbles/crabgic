 ...
 * purpose (this package, `@crabgic/supervisor`, must never depend on either —
 * `packages/detect` depends on `packages/cli`, which depends on
 * `@crabgic/supervisor`; a `@crabgic/supervisor -> @crabgic/detect` edge would close a
 * cycle, and `@crabgic/plugin` is likewise a `packages/cli` dependency this
 ...
 * orchestration layer, which DOES depend on `@crabgic/plugin` and can read
 ...
 * `Pick<EngineAdapter, "capabilities">` (`@crabgic/engine-core` — already this
...
export interface BuildCapabilityManifestOptions {
  ...
  /** 10's own plugin manifest entry (`buildPluginCapabilityEntry`, `@crabgic/plugin`), supplied pre-built by the caller. */
